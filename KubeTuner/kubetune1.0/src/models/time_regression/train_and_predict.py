...
def train_memory_model():
    ...
    model_path = Path(__file__).resolve().parent / 'output' / 'kubetune_rf_model_memoryusage.pkl'
...
def train_cpu_model():
    ...
    search.fit(X_train, y_train)
    model = search.best_estimator_
    model_path = Path(__file__).resolve().parent / 'output' / 'kubetune_rf_model_cpuusage.pkl'
    model_path.parent.mkdir(parents=True, exist_ok=True)
    joblib.dump(model, model_path)
    y_pred = model.predict(X_test)
    print("Best Params:", search.best_params_)
...
def predict_and_export_all():
    ...
    mem_model_file = Path(__file__).resolve().parent / 'output' / 'kubetune_rf_model_memoryusage.pkl'
    ...
    cpu_model_file = Path(__file__).resolve().parent / 'output' / 'kubetune_rf_model_cpuusage.pkl'
