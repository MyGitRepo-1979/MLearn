...
def train_memory_model():
    ...
    file_path = "data/aks_02_data_mb.xlsx"
    ...
    model_path = Path(__file__).resolve().parents[0] / 'output' / 'kubetune_rf_model_memoryusage.pkl'
...
def train_cpu_model():
    ...
    model_path = Path(__file__).resolve().parents[0] / 'output' / 'kubetune_rf_model_cpuusage.pkl'
    model_path.parent.mkdir(parents=True, exist_ok=True)
    joblib.dump(model, model_path)
